...
if len(sys.argv) < 3:
	...
	sys.stderr.write("             use a-z for window mode in 0.5ms increments (commonly f, 3ms)\n\n")
...
while (1):
	...
	if (ver == 'V'):
		...
		if (ord(ver[0]) > ver_max[0] or ((ord(ver[0]) >= ver_max[0]) and (ord(ver[1]) > ver_max[1]))):
...
def fm2_getbits(fm2_str):
    ...
    for x in range(0, 8):
        ...
        if (len(data[3])):p2 = p2 | (0 if (data[3][x] == '.') else (1 << x))
